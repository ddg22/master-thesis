...
\begin{enumerate}
    ...
    \item \textit{StudentDeployer}: as its name said, it is responsible for the deployment of each \textit{Student} contract.
...
\end{enumerate}
It exploits also other four contract, the green ones in \cref{fig:contractsClass}, taken from contracts libraries such as \textit{openzeppelin}\footnote{https://docs.openzeppelin.com/}:
\begin{enumerate}
    \item \textit{EntryPoint}:
    \item \textit{AccessControl}:
    \item \textit{BaseAccount}:
    \item \textit{BasePaymaster}:
\end{enumerate}
